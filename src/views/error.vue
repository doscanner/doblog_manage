        ...
        <div class="tipInfo">
            <div class="in">
                <div class="textThis">
                    ...
                    <p>
                        <span>点击返回
                            <a id="href" href="javascript:void(0)" @click="$router.go(-1)">上一页</a>
                        </span>
                    </p>
                </div>
            </div>
...
export default {
  ...
  methods: {
    init() {
      if (
        util.checkvalue.isnull(this.$route.params) ||
        util.checkvalue.isnull(this.$route.params.code)
      ) {
        return;
      }
      var code = this.$route.params.code.toString();
      if (code == config.statuscode.notfound.toString()) {
        this.msg = "找不到页面！";
      } else if (code == config.statuscode.notpermission.toString()) {
        this.msg = "无权访问该页面！";
      }
    },
    centerWindow(a) {
